...
const JsonReplacer = (): ReactElement => {
	...
	const getInput = async (side: inputSide): Promise<any> => {
      ...
      const [json] = await Promise.all([JSON.parse(inputElem.innerText)]);
	...
	const modifyTarget = (sourceValue: number | string, targetValue: number | string): number | string => {
      ...
      result = sourceValue && (sourceValue === targetValue) ? sourceValue : targetValue;
		return result;
	};
		...
		<div className="jsonReplacerContainer">
			<div className="jsonReplacerHeader">
				<div className="btn-group grp-left">
					<button title="Get the current pages 2d Json" className="btn btn-sm"
							onClick={() => handleGet2dJson(inputSide.left)}>Get 2D
					</button>
					<button title="Get Json from feeder link or a direct json" className="btn btn-sm"
							onClick={() => handleFromClipboard(inputSide.left)}>From Clipboard
					</button>
				</div>
				<div className="btn-group grp-right">
					<button title="Replace left" className="btn btn-sm"
							onClick={() => handleReplace(inputSide.left)}> ←
					</button>
					<button title="Replace right" className="btn btn-sm"
							onClick={() => handleReplace(inputSide.right)}> →
					</button>
				</div>
				<div className="btn-group grp-right">
					<button title="Get the current pages 2d Json" className="btn btn-sm"
							onClick={() => handleGet2dJson(inputSide.right)}>Get 2D
					</button>
					<button title="Get Json from feeder link or a direct json" className="btn btn-sm"
							onClick={() => handleFromClipboard(inputSide.right)}>From clipboard
					</button>
				</div>
			</div>
			...
			<div className="jsonReplacerContents ">
				<div className="jsonReplacerContent mockup-code">
					<pre ref={leftInputElement} contentEditable={true} spellCheck={false}
						 dangerouslySetInnerHTML={{ __html: prettyPrintJson.toHtml(leftJson) }}></pre>
				</div>
				<div className="jsonReplacerContent mockup-code">
					<pre ref={rightInputElement} contentEditable={true} spellCheck={false}
						 dangerouslySetInnerHTML={{ __html: prettyPrintJson.toHtml(rightJson) }}></pre>
				</div>
			</div>
			...
			<div className="jsonReplacerHeader">
				<div className="btn-group grp-left">
					<button title="Get the current pages 2d Json" className="btn btn-sm"
							onClick={() => handleLoadPlan(inputSide.left)}>Load Json
					</button>
					<button title="Get Json from feeder link or a direct json" className="btn btn-sm"
							onClick={() => handleToClipboard(inputSide.left)}>To Clipboard
					</button>
				</div>
				<div className="btn-group grp-right">
					<button title="Replace left" className="btn btn-sm"
				...
				<div className="btn-group grp-right">
					<button title="Get the current pages 2d Json" className="btn btn-sm"
							onClick={() => handleLoadPlan(inputSide.right)}>Load Json
					</button>
					<button title="Get Json from feeder link or a direct json" className="btn btn-sm"
							onClick={() => handleToClipboard(inputSide.right)}>To Clipboard
					</button>
				</div>
			</div>
		</div>
